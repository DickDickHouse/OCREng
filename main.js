const VERSION = "v33-optimized-complete";
...
const basePathWithColor = [
    ...
    [11,1, "blue "], [11,2, "red "], [11,3, "yellow "], [11,4, "green "], [12,5, "blue "], [13,5, "red "],    [14,5, "yellow "], [15,5, "green "], [15,6, "blue "], [15,7, "red "], [15,8, "yellow "], [15,9, "green "],
...
const playerStartIndex = {
    ...
    red: baseIndexByCell.get(xyToIndex(...startStepXY.red)),    green: baseIndexByCell.get(xyToIndex(...startStepXY.green)),
...
function homeList(x1,y1,x2,y2) {
    const list = [];    for (let y = y1; y <= y2; y++) {
...
const players = [
    { id:0, name: "紅方 ", color: "red ", colorClass: "red ", pieces: Array.from({length:4}, ( ,i)=>({status: "home ", homeIndex:i, progress:0})) },
    { id:1, name: "藍方 ", color: "blue ", colorClass: "blue ", pieces: Array.from({length:4}, ( ,i)=>({status: "home ", homeIndex:i, progress:0})) },
    { id:2, name: "綠方 ", color: "green ", colorClass: "green ", pieces: Array.from({length:4}, ( ,i)=>({status: "home ", homeIndex:i, progress:0})) },
    { id:3, name: "黃方 ", color: "yellow ", colorClass: "yellow ", pieces: Array.from({length:4}, ( ,i)=>({status: "home ", homeIndex:i, progress:0})) },
];
...
const POST_TURN_DELAY_MS = 1000;const CAPTURE_BLINK_MS = 2000;
...
function setDiceImage(value) {
    if (diceImageEl) {        diceImageEl.src = `/OCREng/images/dice-${value}.svg`;
...
function initBoard() {
    ...
    boardCells.forEach((cellData) => {        const cell = document.createElement("div");
        ...
        list.forEach(({ player, piece }) => {
            ...
            if (isWaitingForPieceSelection) {
                ...
                if (isValid) {
                    ...
                    pieceEl.onclick = () => handlePieceClick(player, piece);
                }
            }
...
function performMove(player, moveInfo, done) {
    ...
    animateMovePiece(player, moveInfo.piece, moveInfo.target, () => {
        ...
        const enteredHomePath = moveInfo.piece.progress > PATH_LENGTH && (moveInfo.target - moveInfo.steps) <= PATH_LENGTH;
...
function handleTurnEnd(player, moved, dice) {
    ...
    if (extraTurn) {
        ...
        isAnimating = false;
        if (rollButton) rollButton.disabled = false;
        // 不切換 currentPlayerIndex，不呼叫 finalizeTurn    } else {
        // 正常切換玩家
        finalizeTurn();
    }
}
...
function handlePieceClick(player, piece) {
    ...
    isWaitingForPieceSelection = false;
    validMovePieces = [];
    if (rollButton) rollButton.disabled = true; // 移動中禁用按鈕
...
function handleTurn() {
    ...
    const rollInterval = setInterval(() => {
        ...
        if (rollTicks >= maxTicks) {
            ...
            const dice = randomDice();            currentDiceValue = dice; // 存儲點數
            setDiceImage(dice);
            setDiceRolling(false);
            ...
            if (moves.length === 0) {
                ...
                setTimeout(() => handleTurnEnd(player, false, dice), 1000);
            } else if (moves.length === 1) {
                // 只有一種選擇，自動移動 (可選優化：強制點擊體驗更好，這裡為了流暢度自動移動)
...
function initGame() {
    ...
    gameEnded = false;
    isAnimating = false;    isWaitingForPieceSelection = false;
    validMovePieces = [];
    setBlinkingPiece(null);
    initBoard();
    renderPieces();
