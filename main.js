const VERSION = "v54-final-corrected";
...
const basePathWithColor = [
    [2,5, "blue"], [3,5, "blue"], [4,5, "blue"],    [5,4, "green"], [5,3, "blue"], [5,2, "blue"], [5,1, "blue"],
...
function baseIndexToProgress(color, baseIndex) {    return (baseIndex - playerStartIndex[color] + PATH_LENGTH) % PATH_LENGTH;
}
...
const launchRuleSelect = document.getElementById("launch-rule");const confirmRuleButton = document.getElementById("confirm-rule");
...
function updateLaunchRuleDisplay() {
    if (ruleStatusEl) ruleStatusEl.textContent = `起飛規則getLaunchRuleLabel(launchRule)}`;
}
    ...
    cellPiecesMap.forEach((list, cellIndex) => {
        ...
        list.forEach(({ player, piece }) => {
            ...
            if (isWaitingForPieceSelection) {
                ...
                if (isValid) {                    pieceEl.classList.add("selectable");
                    pieceEl.onclick = (e) => {
                        e.stopPropagation();
...
function checkJump(player, piece) {
    if (piece.progress > 0 && piece.progress <= PATH_LENGTH) {
        ...
        if (cellData.classes.includes(playerColorClass)) {
            ...
            if (currentBaseIdx !== undefined) {                for (let i = 1; i < PATH_LENGTH; i++) {
        ...
        if (!safeCells.has(cellIndex)) {
            players.forEach((op) => {
                ...
                op.pieces.forEach((opPiece) => {
                    ...
                    if (opCell === cellIndex) {
                        opPiece.status = "home";
                        opPiece.progress = 0;                        captured.push({ attacker: player, victim: op, piece: opPiece });
                    }
                });
            });
        }
...
function performMove(player, moveInfo, done) {
    ...
    animateMovePiece(player, moveInfo.piece, moveInfo.target, () => {
        ...
        } else {            setStatus(formatStatus(player, `棋子向前 ${moveInfo.steps} 格`));
        }
...
function initGame() {
    players.forEach((player) => {        player.pieces.forEach((p, i) => {
            p.status = "home";
            p.homeIndex = i;
            p.progress = 0;
        });
