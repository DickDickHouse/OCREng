const VERSION = "v44-path-corrected";
...
// ✅ 修正：根據您提供的正確路徑重新定義
// 藍隊路徑：(2,5)>(3,5)>(4,5)>(5,4)>...>(1,8)const basePathWithColor = [
    // 藍隊起飛後的第一步開始，按順時針方向
...
// ✅ 修正：各隊終點隧道
const homePaths = {
    red:   lineCoords(8,2,8,7).map(([x,y]) => xyToIndex(x,y)),    // 紅隊：(8,2)>(8,7)
    yellow:lineCoords(14,8,9,8).map(([x,y]) => xyToIndex(x,y)),   // 黃隊：(14,8)>(9,8)
    green: lineCoords(8,14,8,9).map(([x,y]) => xyToIndex(x,y)),   // 綠隊：(8,14)>(8,9)
    blue:  lineCoords(2,8,7,8).map(([x,y]) => xyToIndex(x,y)),    // 藍隊：(2,8)>(7,8)
};
...
const boardEl = document.getElementById("board");const currentPlayerNameEl = document.getElementById("current-player-name");
...
function getPieceCellIndex(player, piece) {    if (piece.status === "home") return homePositions[player.color][piece.homeIndex];
...
function checkJump(player, piece) {    if (piece.progress > 0 && piece.progress <= PATH_LENGTH) {
...
function applyFlyAndCapture(player, piece) {
    ...
    if (piece.progress > 0 && piece.progress <= PATH_LENGTH) {
        const cellIndex = playerPaths[color][piece.progress - 1];
        if (!safeCells.has(cellIndex)) {            players.forEach((op) => {
...
function formatCaptureMessage(player, capturedList) {    const attackerName = COLOR_NAMES[player.color] || player.name;
...
function performMove(player, moveInfo, done) {
    ...
    animateMovePiece(player, moveInfo.piece, moveInfo.target, () => {
        ...
        if (endProgress === playerPaths[player.color].length) {
            moveInfo.piece.status = "finished";            setStatus(formatStatus(player, "棋子到終點站了!"));
            if (checkWin(player)) {
                done(true, moveInfo.piece);
                return;
            }
